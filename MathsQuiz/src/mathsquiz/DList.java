...
class DList
{
    ...
    public void display(JTextArea myTf)
    {                  // print content of list
        if (head.next == head)
        {             // list is empty, only header Node
            myTf.append("list empty");
            return;
        }
        myTf.append("list content = ");
       // System.out.print("list content = ");
        for (Node current = head.next; current != head; current = current.next)
        {
            String str = current.MathsQuiz.Quiz1.toString()+current.MathsQuiz.Quiz2.toString()+current.MathsQuiz.Quiz3.toString()+"="+current.MathsQuiz.Quiz4.toString()+"   ";
            myTf.append(str);
